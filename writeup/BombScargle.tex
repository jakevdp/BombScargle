...
\jake{mention generalized periodogram, where we fit for the mean as well}
...
\begin{equation}
  O_\omega \equiv \frac{p(\omega|D)}{p(M_0|D)} \propto \exp\left(P_{LS}(\omega)\frac{y^Ty}{2\sigma^2}\right)
\end{equation}
...
\section{Failure of Lomb-Scargle in the Presense of Outliers}
...
\zeljko{Am I missing something here? I understand that the addition of $p_{\rm bad}$ in eq.~\ref{eq:Btheta}
...
\jake{ Yes - we need a robust alternative to the $\chi^2$ as well}

\zeljko{Can we use the same method here as used in the regression problem with straight line plus outliers}

\zeljko{Should we perhaps think of a procedure similar to EM algorithm?}
